...
export function TeacherReviewModal({ gameId, userId }: { gameId: string; userId: string }) {
  ...
  async function onSubmit() {
    ...
    try {
      const response = await fetch("/api/pixa/reviews", {
        ...
        body: JSON.stringify({ gameId, userId, stars, message: message.trim() || undefined, reviewer: "teacher" }),
      });
  ...
  return (
    <div className="rounded-xl border border-white/15 bg-white/10 p-4">
      <p className="text-sm font-extrabold text-white">{he.host.reviewPrompt}</p>
